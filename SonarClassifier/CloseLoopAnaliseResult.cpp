...
/**
 * @brief CloseLoopAnaliseResult::processDeepLearningResults
 * @param csvGTName
 * @param csvResultsName
 */
void CloseLoopAnaliseResult::processDeepLearningResults(const char *csvGTName, const char *csvResultsName)
{
    loadGtMatch(csvGTName);
//    remapGtMatch();
    loadDirectResult(csvResultsName,gtGraph.size());
...
void CloseLoopAnaliseResult::loadFrameDescriptionInformation(const char *fileName)
{
    frResults.clear();
...
void CloseLoopAnaliseResult::remapGtMatch()
{
    unsigned idMax=0;
    remap.resize(gtGraph.size());

    for(unsigned u = 0 ; u < gtGraph.size();u++)
    {
        if(gtGraph[u].size() > 0 )
        {
            remap[u] = idMax;
    ...
    for(unsigned u = 0; u < gtGraph.size();u++)
    {
        if(gtGraph.size() == 0) continue;
        for(unsigned i = 0 ; i < gtGraph[u].size() ; i++)
            gtGraph[u][i].first = remap[gtGraph[u][i].first];

        if(u != remap[u])
        {
            gtGraph[remap[u]] = gtGraph[u];
        }
    }
    cout << "Remapped from " << gtGraph.size() << " vertexs to " << (idMax+1) << " vertex" << endl;
    gtGraph.resize(idMax+1);
}
...
void CloseLoopAnaliseResult::loadAnalisyAndSave()
{
    ...
    loadFrameDescriptionInformation("Results/ResultFramesInformations.csv");
...
void CloseLoopAnaliseResult::generateGTImage()
{
    ...
    Mat gtImage(nFr,nFr, CV_8UC1, Scalar(0));
...
void CloseLoopAnaliseResult::generateResultImage()
{
    ...
    Mat resultImage(resultGraph.size(),resultGraph.size(), CV_8UC1,Scalar(0));
...
void CloseLoopAnaliseResult::generatePairFeaturesCount()
{
    ...
    Mat frameInfImage(resultGraph.size(),resultGraph.size(), CV_8UC1,Scalar(0));
