...
/**
 * @brief Drawing::drawMatchings - Draw match lines between two images on one.
 * @param BGRImg - Resulted image.
 * @param el - Translation and scale of first image.
 * @param er - Translation and scale of second image.
 * @param leftPts - Match points in left image.
 * @param rightPts - Mtach poins in right image.
 * @param color - Line colors.
 */
void Drawing::drawMatchings(Mat &BGRImg, const Scalar_<float> &el, const Scalar_<float> &er,
                            vector<Point2f> &leftPts, vector<Point2f> &rightPts, const Scalar &color)
{
    for(unsigned i = 0 ; i < leftPts.size(); i++)
    {
        line(BGRImg,
             Point(el.val[2] + el.val[0]*leftPts[i].x, el.val[3] + el.val[1]*leftPts[i].y),
             Point(er.val[2] + er.val[0]*rightPts[i].x, er.val[3] + er.val[1]*rightPts[i].y),
             color,2);
    }
}
...
{
    unsigned aw = 50, // Axis width
             ah = 50, // Axis height
             aEndsz = 20,  // Lenght of arrow head

             // Spaces on plot area
             upSpace = 10, downSpace = 10,
             leftSpace = 10, rightSpace = 10,

             pw = chart.m_width-aw-rightSpace-leftSpace-aEndsz, // Plot width
             ph = chart.m_height-ah-upSpace-downSpace-aEndsz;// Plot height
    ...
    result = Mat(chart.m_height, chart.m_width,
                 CV_8UC3, Scalar(255,255,255));
    ...
    // Drawing axis
    drawArrow(result, Point2f(aw, chart.m_height-ah),
              0.f, chart.m_width-aw,Scalar(0,0,0),
              5,aEndsz,1);

    drawArrow(result, Point2f(aw, chart.m_height-ah),
              -90.f,chart.m_height-ah,Scalar(0,0,0),
              5,aEndsz,1);

    // Axis resolution (in total values on axi)
    int hResolution = 8,
        vResolution = 4;

    float aEh = pw/hResolution,
          aEv = ph/vResolution,
          vP = chart.m_height-ah-downSpace,
          hP = aw + leftSpace;

    char tempStr[100];

    // Vertical axis
    for(unsigned i = 0; i <= vResolution; i++)
    {
        float cv = i*aEv;
        // Lines
        line(result,
         Point2f(aw,vP-cv),
         Point2f(aw/2.f,vP-cv),
           Scalar(0,0,0),1);
//        vP+=aEv; // lower precision with sum! Replaced by multiplication operator

        // Labels
        sprintf(tempStr, "%.3g", cv/ey+my);
    ...
    // Horizontal axis
    for(unsigned i = 0; i <= hResolution; i++)
    {
        float ch = i*aEh;
        // Lines
        line(result,
         Point2f(hP+ch,chart.m_height-ah),
         Point2f(hP+ch,chart.m_height-ah/2.f),
           Scalar(0,0,0),1);
//        hP+=aEh; // lower precision with sum! Replaced by multiplication operator

        // Labels
        sprintf(tempStr, "%.3g", ch/ex+mx);
...
/**
 * @brief Drawing::drawPoly - Draw a line beteween point i and i+1.
 * @param img - BGR image
 * @param pts - Point set (polly)
 * @param color - Line colors
 */
void Drawing::drawPoly(Mat &img, vector<Point2f> &pts, const Scalar &color)
{
    if(pts.size() < 2)
    {
        cout << "Drawing::drawPoly - Error polly with no points" << endl;
        return;
    }
    unsigned n = pts.size();
    line(img, pts[n-1], pts[0],color);
    n--;
...
/**
 * @brief Drawing::getSonarShape - Return a point set of the sonar shape
 * @param p - Sonar position
 * @param bearing - Sonar bearing (degrees)
 * @param range - Sonar range (pixels)
 * @param heading - Sonar heading/orientation/pitch
 * @param pts - Return computed points
 * @param nPts - Number of points
 */
void Drawing::getSonarShape(const Point2f &p, float bearing, float range,
                            float heading,
                            vector<Point2f> &pts, unsigned nPts)
{
    if(nPts < 3) return;
    pts.resize(nPts);
    ...
    for(float currentRad = rad + radBearing/2.f;
        pId < nPts; currentRad-= radInc)
    {
        pts[pId] = p + Point2f(sin(currentRad), cos(currentRad))*range;
        pId++;
    }
}

/**
 * @brief Drawing::bouldingBox - Find min and max value for x and y in a point set.
 * @param pts - Point set input
 * @param mp - Min values (x, y) output
 * @param Mp - Max values (x, y) output
 */
void Drawing::bouldingBox(const vector<Point2f> &pts, Point2f &mp, Point2f &Mp)
{
    mp.x = Mp.x = pts[0].x;
    mp.y = Mp.y = pts[0].y;
    for(unsigned i = 1; i < pts.size() ; i++)
    {
        if(mp.x > pts[i].x) mp.x = pts[i].x;
        if(mp.y > pts[i].y) mp.y = pts[i].y;
...
void Drawing::drawArrow(Mat &bgrImg,
                        const Point2f &p, float deg, float lenght,
                        const Scalar &color, int opennig, int endSz,
                        int thickness)
{
    if(opennig == -1) opennig = lenght*0.2f;
    if(endSz == -1) endSz = lenght*0.1f;

    float rad = deg * M_PI/180.f;
    Point2f v(cos(rad),sin(rad)), vOrto(v.y,-v.x),
            pEnd(p+lenght*v), pHead(pEnd-endSz*v),
            p1(pHead + opennig*vOrto),
            p2(pHead - opennig*vOrto);

    line(bgrImg,p, pEnd, color,thickness);
    line(bgrImg,p1, pEnd, color,thickness);
    line(bgrImg,p2, pEnd, color,thickness);
}

/**
 * @brief Drawing::imShow16BitsTrucated
 *   Show an image 16 bits image truncate in 8bits
 * using OpenCV highgui.
 * @param windowName - OpenCV highgui window name.
 * @param img16b - 16bit grayscale image.
 * @param offset - trucation offset (default = 0). Usual negative values.
 */
void Drawing::imShow16BitsTrucated(const string &windowName, Mat &img16b, unsigned offset)
{
    Mat img8b;
    img16b.convertTo(img8b,CV_8UC1,
                     1.0, // Aways one because truncation
                     offset);
    imshow(windowName,img8b);
}
