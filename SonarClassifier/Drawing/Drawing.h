...
public:
    ...
    static void drawMatchings(Mat &BGRImg, const Scalar_<float> &el, const Scalar_<float> &er,
                              vector<Point2f> &leftPts, vector<Point2f> &rightPts,
                              const Scalar &color);
    ...
    static void drawPoly(Mat &img, vector<Point2f> &pts, const Scalar &color);

    static void getSonarShape(const Point2f &p, float bearing, float range,
                              float heading,
                              vector<Point2f> &pts, unsigned nPts);

    static void bouldingBox(const vector<Point2f> &pts, Point2f &mp, Point2f &Mp);


    static void drawArrow(Mat &bgrImg,
                          const Point2f &p, float deg, float lenght,
                          const Scalar &color, int opennig =-1, int endSz=-1,
                          int thickness=1);

    static void imShow16BitsTrucated(const string &windowName, Mat &img16b,
                                     unsigned offset=0);
