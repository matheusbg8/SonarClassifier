...
GroundTruth::GroundTruth():
    ...
    groundTruthFileName("Untitled.txt"),
//    windowSize(1850,910),
    windowSize(1300,600),
...
void GroundTruth::saveEmptyFrames(const char *fileName)
{
    FILE *f = fopen(fileName,"w");
    if(f == 0)
        return;
...
void GroundTruth::loadGroundTruth(const char *fileName)
{
    ...
    frames.resize(frameSize,FrameGT(string(""),0));
    for(frame = 0 ; frame < frames.size() ; frame++)
    {
        FrameGT &fr = frames[frame];
...
bool GroundTruth::loadEmptyFrames(const char *fileName)
{
    ...
    {
        frames.push_back(FrameGT(string(img_file_name),frame++));
    }
    fclose(f_img_names);
...
//            clearNoMatchFrames();
        ...
        break;
        case 'q': // Automatic go next frame
            autoGo = !autoGo;
        break;
...
bool GroundTruth::findMatchByIntersectGaussian(unsigned ufr, Gaussian &u, unsigned vfr,
                                               Gaussian **uFound, Gaussian **vFound)
{
    #ifdef GROUND_TRUTH_TRACKING_DEBUG
        cout << "GroundTruth:findMatchByIntersectGaussian:: use!" << endl;
    #endif

    int guID, gvID;
    FrameGT &uf = frames[ufr], &vf = frames[vfr];

    uf.findIntersectGaussian(u,&guID);
    if(guID>=0)
        (*uFound) = &(uf.gaussians[guID]);
    ...
    *vFound = &(vf.gaussians.at(gvID));

    return true;
}

/**
 * @brief GroundTruth::findMatchByIntersectGaussian - Try to find match between gaussians, given
 * a gaussian to search in a given souce frame ID. Also a destin frame ID is needed.
 *
 * @param uFrId - Frame ID to search the Gaussian.
 * @param u - Given Gaissian to search.
 * @param vFrId - Destin frame ID (input)
 * @param uIdFound - Gaussian found in src frame (uFrId) (output)
 * @param vIdFound
 * @return - true if a matching is found.
 */
bool GroundTruth::findMatchByIntersectGaussian(unsigned uFrId, Gaussian &u,unsigned vFrId,
                                               int *uIdFound, int *vIdFound)
{
    ...
    *vIdFound=-1;
    ...
    FrameGT &uFr = frames[uFrId];

    uFr.findIntersectGaussian(u,uIdFound);
    if(*uIdFound == -1)
        return false; // Gaussian was not found

    uFr.findMatch(*uIdFound,vFrId,vIdFound);
    if(*vIdFound == -1)
        return false; // No match registred for this gaussian

    return true; // Gaussian and match were found
}

/**
 * @brief   Find a gaussian close to (ux, uy) on uFrame
 *  considering a precision. If a gaussian was found
 * ux and uy will be changed to position of gaussian
 * found on uFrame and vx and vy will be changed
 * with position of a gaussian on vFrame that match
 * with the found gaussian on uFrame.
 *
 * @param ufr - uFrame ID
 * @param ux - x position to search
 * @param uy - y position to search
 * @param vfr - vFrame ID
 * @param vx - It will be filled with a gaussian postion
 * of vFrame.
 * @param vy - It will be filled with a gaussian postion
 * of vFrame.
 * @param prec - Distance precision that will be used.
 * @return bool - true if a pair match of gaussian was found.
 */
...
//    segmentation.createRTPlot(imgFr1);
    ...
    if(showGaussians)
    {
        ...
        {
            drawGaussian(cimg1,gFr1[i],Scalar(0,0,255),Scalar(0,255,0),i,true,false,true);
        }
        for(unsigned i = 0 ; i < gFr2.size(); i++)
        {
            drawGaussian(cimg2,gFr2[i],Scalar(0,0,255),Scalar(0,255,0),i,true,false,true);
        }
    }
    ...
    if(showColorMosaic)
    {
        matchHandler.createColorMosaicImage(cimg1,cimg2);
        showColorMosaic=false;
    }
...
void GroundTruth::createFrameRelationGraph(vector<vector<unsigned> > &graph)
{
    typedef pair<unsigned, unsigned> PUU;

    uchar matGraph[frames.size()][frames.size()];
    memset(matGraph,0,frames.size()*frames.size()*sizeof(uchar));

    cout << "Creating frame relation graph..." << endl;

    for(unsigned i = 0; i < frames.size(); i++)
    {
        vector< vector<PUU> > &matches = frames[i].match;

        for( unsigned j = 0 ; j  < matches.size() ; j++)
        {
            for(unsigned k = 0 ; k < matches[j].size(); k++)
            {
                PUU &dest = matches[j][k];
                matGraph[dest.first][j] = 1;
                matGraph[j][dest.first] = 1;
            }
        }
    }

    // copying the graph to answer O(V^2)
    graph.clear();
    graph.resize(frames.size());
...
void GroundTruth::clearNoMatchFrames()
{
    cout << "Clearing no Match Gaussian" << endl;
    uchar frameHasMatchRelation[frames.size()];
    memset(frameHasMatchRelation,0,frames.size()*sizeof(uchar));

    for(unsigned i = 0; i < frames.size(); i++)
    {
        vector< vector<PUU> > &matches = frames[i].match;

        cout << "Looking for frame "
             << i
             << endl;
        ...
        for( unsigned j = 0 ; j  < matches.size() ; j++)
        {
            ...
            {
                PUU &dest = matches[j][k];
                frameHasMatchRelation[j] = 1;
                frameHasMatchRelation[dest.first] = 1;
            }
        }
    }
    ...
    for(unsigned  i = 0 ; i < frames.size() ; i++)
    {
        if(!frameHasMatchRelation[i])
        {
            cout << "Clearing frame gaussian "
                 << i
                 << endl;
...
//    medianBlur(imgFr2,imgFr2,3);
    ...
    matchHandler.loadMatchs(frames[currentFrame0],frames[currentFrame1],
                            imgFr1, imgFr2,10.f);
...
void GroundTruth::selectGaussOnFrame(int x, int y, int *frame, int *gauss)
{
    ...
    {
        *frame = 0;
        frames[currentFrame0].findClosestGaussian((x-el.val[2])/el.val[0],(y-el.val[3])/el.val[1], gauss);
    }else
    {
        *frame = 1;
        frames[currentFrame1].findClosestGaussian((x-er.val[2])/er.val[0],(y-er.val[3])/er.val[1], gauss);
    }
}
...
void GroundTruth::acceptHomographyMatchs()
{
    ...
    FrameGT &currentFrame = frames[currentFrame0];
...
void GroundTruth::createGaussian(FrameGT *frame, Mat &img16Bits)
{
    #ifdef GROUND_TRUTH_TRACKING_DEBUG
        cout << "GroundTruth:createGaussian:: use!" << endl;
    #endif
 ...
 /// @todo - Ground Truth change thresholds
//    segmentation.searchThreshold = threshold1;
//    segmentation.pixelThreshold = threshold2;
...
//    segmentation.acceptPeakHeight = threshold2;
//    segmentation.searchDistance = threshold1;
...
void GroundTruth::createGaussian(Segment *seg, Gaussian *g)
{
    ...
    g->intensity = mMean.at<float>(2,0);

    g->dx = sqrt(eVal.at<float>(2,0));
    g->dy = sqrt(eVal.at<float>(1,0));
    g->di = sqrt(eVal.at<float>(0,0));
    g->N = seg->N;
