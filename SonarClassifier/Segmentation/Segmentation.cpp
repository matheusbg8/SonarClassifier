...
// Respect counter clock wise order!! Do not change this values
const int Segmentation::neighborRow[8] = { -1, -1,  0,  1,  1,  1, 0, -1};
const int Segmentation::neighborCol[8] = {  0, -1, -1, -1,  0,  1, 1,  1};
//                      Compass Labels = {  N, NW,  W, SW,  S, SE, E, NE}
// Diagonal neighbors = odd indexes | Horizontal/Vertical neighbors = even indexes
...
#include "SegmentExtractor/OrderedBorderSegmentExtractor.h"
...
void Segmentation::load(ConfigLoader &config)
{
    ...
    if(config.getString("General","SegmentExtractor",&str))
    {
        ...
        }else if(str == "OrderedBorderSegmentExtractor")
        {
            se = new OrderedBorderSegmentExtractor;
