...
//    Mat tmp;
//    img16bits.convertTo(tmp,CV_8UC1);
//    imshow("LinearSeg:ResivedImg", tmp);
    imshow("LinearSeg:ResivedMask", *imgMask);
    waitKey();
//    tmp.release();
#endif
    ...
    for(unsigned row = 0 ; row < img16bits.rows ; row+=rowJump)
    {
        ...
        {
            ushort p = img16bits.at<ushort>(row,col) ;
    ...
    cout << "ROI Extraction Result:" << endl
         ...
         << endl;
    waitKey();
    imshow("LinearSeg:ResultedSearchMask", *searchMask);
    waitKey();
