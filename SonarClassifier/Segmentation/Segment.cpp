...
/**
 * @brief Segment::toCVContour
 *  Convert the segment to OpenCV contour data format
 * @param mBin - Conversion result
 */
void Segment::toCVContour(Mat &mBin)
{
    mBin = Mat(NCountor,1, CV_32SC2);

    for(unsigned i = 0; i < NCountor;i++)
    {
        mBin.at<int>( i, 0) = countor.at<ushort>(1,i);
        mBin.at<int>( i, 1) = countor.at<ushort>(0,i);
    }
}
 ...
 * @brief Segment::toCVContour
 *  Convert the segment to OpenCV contour data format
 * @param contour - Conversion result
 */
void Segment::toCVContour(vector<Point> &contour)
{
    contour.resize(NCountor);
    for(unsigned i = 0; i < NCountor;i++)
    {
        contour[i].x = this->countor.at<ushort>(1,i);
        contour[i].y = this->countor.at<ushort>(0,i);
    }
}
