...
void SingleImageWindow::propagateMouseEvent(int event, int flags, int x, int y)
{
    for(unsigned i = 0 ; i < sfts.size(); i++)
    {
        sfts[i]->mouseEvent(event, flags, x, y);
    }
}
...
void SingleImageWindow::resetScren(Scalar color)
{
    screen = Mat(windowSize.height,windowSize.width,CV_8UC3, color);
}
...
{
    propagateMouseEvent(event,flags,x,y);
