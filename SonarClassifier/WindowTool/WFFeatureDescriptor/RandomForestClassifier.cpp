...
void RandomForestClassifier::training(Mat &labels, Mat &data)
{
...
//                   TermCriteria termCrit );
    CvRTParams pr = CvRTParams( );

    pr.max_depth = 5;
    pr.min_sample_count = 10;
    pr.regression_accuracy = 0;
    pr.use_surrogates = false;
    pr.max_categories = 10;
    pr.priors = 0;
    pr.calc_var_importance = true;
    pr.nactive_vars = round(sqrt(nvars));
    pr.term_crit = TC(100,0.01f);

//    randomTrees.train(data,CV_ROW_SAMPLE,labels);
    randomTrees.train(data,CV_ROW_SAMPLE,labels,Mat(),sample_idx,var_type,Mat(),pr);
    isTrained = true;

    cout << "Number of trees: " << randomTrees.get_tree_count() << endl;

    cout << "Var importances:" << endl;
    Mat varImp = randomTrees.getVarImportance();

    for(unsigned i = 0; i < varImp.cols; i++)
    {
...
void RandomForestClassifier::predict(Mat &data, Mat &predictions)
{
    int nsamples_all = data.rows;
    ...
    for(int i = 0; i < nsamples_all; i++ )
    {
        ...
        predictions.at<float>(i,0) = randomTrees.predict(sample);
    }
}

double RandomForestClassifier::predict(Mat &data)
{
    return randomTrees.predict(data);
}

bool RandomForestClassifier::trained()
{
    return isTrained;
}
