...
//    roboMap("../../../../SonarGraphData/Maps/Yatcht_Special.ini"),
    roboMap("../../../../SonarGraphData/Maps/Yatcht_Special_Small.ini"),
...
bool WFGTLoop::loadGTLoopWithTimeStamp(const string &file)
{
    ...
    if(f == 0x0)
    {
        cout << "WFGTLoop:: GTLoop can't load GTLoop and timestamp dataset: "
             << file
             << endl;
        return false;
    }
...
bool WFGTLoop::saveGTLoop(const string &fileName)
{
    ...
    if(f == 0x0)
    {
        cout << "WFGTLoop:: GTLoop can't save GTLoop dataset: "
             << fileName
             << endl;
        return false;
    }
...
//    csvLoader.loadHeadingFromCSV ( datasetPath + "compass.csv" );
    csvData.loadFramesFromCSV  ( datasetPath + "imgs/sonar_xy_gray_info.csv");
...
void WFGTLoop::processImages(Mat &leftImg, int leftId, Mat &rightImg, int rightId)
{
    ...
    cout << "Left image: ID " << leftId << " p: " << leftFr.p << " heading " << leftFr.heading << endl;
    cout << "Right image: ID " << rightId << " p: " << rightFr.p << " heading " << rightFr.heading << endl;
