                                       ...
                                       vector<MatchInfo> &result)
{
    ...
    unsigned i,j; // currentId , nextId

    for( i = (basedMatchId+1)%nMatchs, j = (i+1)%nMatchs;
                     j != basedMatchId;
                    i=j, j=(j+1)%nMatchs
       )
    {
        ...
        if( fabs(
              ...
              dist( ref2 , g2[current.vID ] ) )
                ...
                &&
             fabs(
               angleBetween(g1[current.uID],ref1,g1[next.uID])
                          -
               angleBetween(g2[current.vID],ref2,g2[next.vID]) )
                         <= 15.f  // degree
           )
        {
...
//        fastMatchCheck2(sd1,sd2,vertexMatch,i,filterResult);
